...
More than likely, you are going to want to use another package to analyze the simulated data produced by `{csPointPros}`. Here, we will show how the output from `{csPointPros}` works seemlessly with the spatial analysis package, `{spatialTIME}`
...
sim_object = GenerateTissue(sim_object, density_heatmap = F) %>%
    #create holes
    GenerateHoles(density_heatmap = F) %>%
