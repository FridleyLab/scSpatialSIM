---
title: "Simulating Univariate Cells"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Simulating Univariate Cells}
...
```{r}
sim_object = CreateSimulationObject(sims = 9, cell_types = 1)
```
...
The tissue kernel slot is now filled the number of kernels matching number of spatial patterns we have. Let's take a look at the simulated tissue kernels with `PlotSimulation`.
...
The parameters that go into `GenerateHoles` is similar to that of `GenerateTissue` with one addition: `hole_prob` or the proportion range of the point patterns that could become holes. All of these parameters had defaults set when the spatial simulation object was created, but can be overridden here is a particular area of the pattern is wanted to have the holes and not the rest. The number of holes is random as long as the sum of the area is within the proportions above. 
...
For univariate clustering, smaller standard deviations and higher probability range will increase the amount of clustering. Large standard deviations and low probabilities will make the clustering metrics low. For univariate cell simulation, the correlation metric does nothing.
