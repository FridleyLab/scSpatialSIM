  ...
  dmb = lapply(seq(ncells), function(cell){
    ...
    if(density_heatmap){
      if(cell == 1){
        ...
        sim_object@Cells[[cell]]@`Density Grids` <<- sim_object@Cells[[1]]@`Density Grids`
      }
    ...
    sim_object@`Spatial Files` <<- pbmcapply::pbmclapply(seq(sim_object@`Spatial Files`), function(spat_num){
      vec = CalculateGrid(sim_object@`Spatial Files`[[spat_num]],
                                                      sim_object@Cells[[cell]]@`Simulationed Kernels`[[spat_num]], cores = cores) * 0.9
      df = data.frame(col1 = scale_probs(vec, params$probs))
